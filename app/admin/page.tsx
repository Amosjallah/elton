  ...
  useEffect(() => {
    async function fetchDashboardData() {
      try {
        ...
        // 5. Fetch Top Products based on order_items
        ...
        if (productData) {
          ...
          salesData?.forEach(item => {
            ...
            salesMap[item.product_name].sales += item.quantity || 0;
            salesMap[item.product_name].revenue += (item.price || 0) * (item.quantity || 0);
          });
          ...
          const processedProducts = productData.map((p: any) => ({
            ...
            sales: salesMap[p.name]?.sales || 0,
            revenue: salesMap[p.name]?.revenue || 0,
            stock: p.quantity
          }));

          // Sort by sales descending
          processedProducts.sort((a: any, b: any) => b.sales - a.sales);
          setTopProducts(processedProducts.slice(0, 4));
        }
